...
**New this session: "Median Cycle Time by RFP Week" is now a combo chart.** The person noticed the median trending sharply down in recent weeks and asked, in effect, "is that real speed, or is the team just clearing recently-RFP'd spools while an older week's batch still sits open?" `src/painting/summary.py -> build_weekly_trend()` now also returns `open_count` per RFP-week bucket (spools RFP'd that week, still not PDI cleared — same definition as the KPI strip's own "Still Open"), and `painting-charts.js -> renderTrend()` plots it as a second bar series on a right-hand axis alongside the existing median-days line. Verified against the real published bundle: it confirms the person's suspicion — Week 19→23's median drops 14→10→8→null→null working days while open_count for those same weeks climbs 76→189→229→265→266. Full reasoning and verification method in the CHANGELOG entry.
