...
**New this session: "Median Cycle Time by RFP Week" is now a combo chart.** The person noticed the median trending sharply down in recent weeks and asked, in effect, "is that real speed, or is the team just clearing recently-RFP'd spools while an older week's batch still sits open?" `src/painting/summary.py -> build_weekly_trend()` now also returns `open_surface_area` per RFP-week bucket (surface area of spools RFP'd that week, still not PDI cleared — `open_count` also kept for tooltip context), and `painting-charts.js -> renderTrend()` plots the surface area as a second bar series on a right-hand axis alongside the existing median-days line (bar metric corrected from spool count to surface area same session, per the person: "instead of spool count, please show surface area"). Verified against the real published bundle: it confirms the person's suspicion — Week 19→23's median drops 14→10→8→null→null working days while that same population's still-open surface area climbs 76.4→189.2→375.1→546.2→381.3 m². Full reasoning and verification method in the CHANGELOG entries (two, same day).

**Publishing status: NOT YET on `origin/main`.** This change is committed on a local branch (`feature/painting-trend-open-count`, in a separate git worktree at `../lss-worktree-trend` alongside this checkout) but was not pushed — needs the person's go-ahead first, since it touches the live site. If picking this back up: check whether that branch/worktree still exists and whether it's already been merged before redoing the work.
