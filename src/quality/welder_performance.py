...
def build_project_wise_summary(dataframe: pd.DataFrame) -> list[dict[str, Any]]:
    """Chart: "Project Wise Summary" - Total/Accept/Reject NDT joints per project."""

    grouped = dataframe.groupby("Project Name").agg(
        total_joint=("Total NDT Joint", "sum"),
        accept_joint=("NDT Accept Joint", "sum"),
        reject_joint=("Rejected Joint", "sum"),
    )
    ...
    rows = []
    for project, row in grouped.iterrows():
        total = float(row["total_joint"] or 0)
        reject = float(row["reject_joint"] or 0)
        rows.append({
            "project": project,
...
def build_bundle(dataframe: pd.DataFrame) -> dict[str, Any]:
    return {
        ...
        "project_wise": build_project_wise_summary(dataframe),
