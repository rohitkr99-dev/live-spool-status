"""
src/welding_finish.py
---------------------------------------------------------
Derives a "Welding Finish" date per spool - the date the LAST joint
finished welding. Originally built (2026-07-30) as a Production-
dashboard-only, additive calculation, deliberately separate from the
Projects pipeline's existing "First Welding" (src/merge.py's
summarize_first_activity() - the EARLIEST Activity Date per spool in
the Weekly workbook's Fit-Up DB / Welding DB, used to mark the START
of the Welding stage, not its completion).

PROMOTED to this shared top-level location 2026-08-15 (given by the
person, in their own words: "the rules under Production pages are
actual and Projects numbers should have been updated as per that
only") - the Projects dashboard's "Welding" stage (config/
stages.json) now uses THIS module's stricter, all-joints-done result
as its own date_field too (see merge.py -> apply_welding_finish()),
instead of the looser "First Welding" field, which stays computed
and available on the master dataset unchanged (nothing reads it for
stage-gating any more, but nothing that already depended on it for
anything else was touched). Both dashboards now compute Welding
Finish through this exact same function, from the exact same source
data - the numeric mismatch this fixes was never a bug in either
pipeline individually; it was two independently-built, genuinely
different definitions of "Welding done" that had never been
reconciled.
