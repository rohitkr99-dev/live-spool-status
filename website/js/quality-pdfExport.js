...
const QualityPdfExport = {

  /** One entry per <section> on the page that holds chart cards -
   * anything without a ".chart-card canvas" is skipped automatically. */
  collectSections() {
    const sections = [];
  ...
  async export() {
    ...
    if (typeof QualityData === "undefined" || !QualityData.hasData) {
      QualityApp.showToast("No data loaded yet - nothing to export", true);
      return;
    }
    ...
    if (!sections.length) {
      QualityApp.showToast("No charts to export yet", true);
      return;
    }
    ...
    doc.text("Quality Assurance / Control", margin, y);
    y += 6;
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("DEE Piping Systems — Quality Assurance / Control", margin, pageHeight - 20);
    }

    const stamp = new Date().toISOString().slice(0, 10);
    doc.save(`quality-charts-${stamp}.pdf`);
  },
  ...
  init() {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error(error);
        QualityApp.showToast("Couldn't build the PDF - see console for details", true);
      } finally {
...
document.addEventListener("DOMContentLoaded", () => QualityPdfExport.init());
