...
const ProductionPdfExport = {
  ...
  async export() {
    ...
    if (typeof ProductionData === "undefined" || !ProductionData.hasData) {
      ProductionApp.showToast("No data loaded yet - nothing to export", true);
      return;
    }
    ...
    if (!sections.length) {
      ProductionApp.showToast("No charts to export yet", true);
      return;
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("DEE Piping Systems — Production", margin, pageHeight - 20);
    }

    const stamp = new Date().toISOString().slice(0, 10);
    doc.save(`production-charts-${stamp}.pdf`);
  },
  ...
  init() {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error(error);
        ProductionApp.showToast("Couldn't build the PDF - see console for details", true);
      } finally {
...
document.addEventListener("DOMContentLoaded", () => ProductionPdfExport.init());
