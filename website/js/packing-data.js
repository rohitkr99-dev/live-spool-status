...
const PackingData = {
   ...
   * packing-app.js -> loadInitialData()/showEmptyState()).
   */
  async fetchPublished() {
    ...
    if (!response.ok) throw new Error(`Published packing data returned ${response.status}`);

    let bundle;
    try {
      bundle = await response.json();
    } catch (error) {
      throw new Error("Published packing data isn't valid JSON: " + error.message);
    }
    ...
    try {
      ...
      throw new Error("Published packing data is unreadable: " + error.message);
    }
