...
### 2026-09-07 - Painting: "Median Cycle Time by RFP Week" now also shows how many of that week's spools are still open

The person, looking at the chart's recent weeks trending toward a much lower median: "There might be a case that reducing median cycle time is showing that now more close RFP spools are being cleared from painting. But there could also be a case that painting team is deliberately picking up recent spools instead of old ones. I want to see these both trends together." First asked about overlaying the Painting Plan's own "Planned Week" field - stopped mid-explanation to reframe: "Is it possible to show remaining spools of those RFP cleared weeks in the same chart? Using same Week bucket of RFP week? ... The final chart can be combo chart with dual axis." I.e. for the SAME RFP-week bucket the median-days line already uses, also show how many spools RFP'd that same week are still sitting open (not yet PDI cleared) - so a shrinking median next to a growing "still open" bar reads as backlog/cherry-picking, not genuine speed.

No new data source needed - `is_complete` (`pdi_clearance is not None`) and `rfp_date` are already on every merged record. `src/painting/summary.py -> build_weekly_trend()`: now splits each RFP-week bucket into `completed_by_week` (unchanged: median of `total_cycle_days`, only where `is_complete` and the value is present/non-negative) and `open_by_week` (count of `not is_complete` records with that RFP week) - the exact same `open_count` definition `build_kpi_summary()`'s own "Still Open" KPI already uses, just re-grouped per week instead of totalled once. Weeks with open spools but zero completions yet (e.g. the most recent 1-2 weeks, nothing's had time to clear) now appear in the list too, with `median_days: null` - previously those weeks were simply absent from the chart.

`website/js/painting-charts.js -> renderTrend()`: switched to a mixed Chart.js dataset (same combo-chart pattern already used by `quality-charts.js -> renderWelderMonthJoint()` - per-dataset `type`, `yAxisID`, `order`) - a new bar series "Still open (RFP'd that week)" on a new right-hand `y1` axis (DEE blue, `PAINTING_CONFIG.stageColor`), alongside the existing median-cycle-days line (left `y` axis, unchanged red) and the 4-day-ideal dashed line. Tooltip's `afterTitle` now reports both counts together. `website/painting.html`: updated the chart's hint text to explain the combo reading directly ("a shrinking median with a growing bar isn't speed, it's backlog"); bumped `painting-charts.js` to `?v=20260907`.

Verified against the real published bundle (not a synthetic one) - patched a scratch copy of the live `b3f7e6a1d4.json`'s `weekly_trend` using the actual new `build_weekly_trend()` (not a reimplementation) and served it locally (auth-guard stripped from that scratch copy only, to preview without Firebase login). Confirms the person's suspicion #2 was right, not #1: Week 19-23 median drops 14 -> 10 -> 8 -> null -> null working days while `open_count` for those same weeks climbs 76 -> 189 -> 229 -> 265 -> 266 - the recent "improvement" is mostly unfinished backlog sitting behind a small fast-cleared minority, not the team actually getting faster. Confirmed via Chart.js instance inspection (dataset types/axes/values) and a screenshot of the rendered chart; full page loaded with no console errors and `body.is-ready` set (no regression to the rest of the page).

### 2026-09-07 (cont'd) - Same chart: bar now plots surface area, not spool count

Immediately after the above, the person: "instead of spool count, please show surface area" - the bar should reflect the paint shop's actual open WORKLOAD (a small fitting and a large spool aren't equal), matching the same count-vs-surface-area distinction the "Quantum of Work Pending" chart's own metric toggle already draws (2026-09-05 entry).

`build_weekly_trend()`: added `open_area_by_week` (sums each open record's `surface_area`, same `... or 0` null-handling as `PaintingPendingWork.metricValue()` on the frontend) alongside the existing `open_count_by_week` - both are still returned (`open_count` kept for the tooltip's spool-count context, `open_surface_area` is the new field the chart itself plots). `renderTrend()`: the bar series now reads `open_surface_area`; right axis retitled "Still open (surface area, m²)"; tooltip's `afterTitle` reports both together, e.g. "137 spool(s) cleared · 229 still open (375.1 m²), RFP'd that week". `painting.html` hint text updated to match; `painting-charts.js` version bumped again, `?v=20260907b`.

Re-verified the same way as the first pass (real bundle, actual `build_weekly_trend()`, local preview, Chart.js instance inspection + screenshot) - Week 21's bar now reads 375.1 m² (229 spools) instead of the bare count, chart renders with no console errors.

### 2026-09-07 (cont'd) - Same chart: added a second "still pending" bar, grouped by Planned Week instead of RFP Week

Asked (after discussing whether this chart could pull from DPR instead of Painting's own workbook - it already does, see the entry below this one): "Is it more possible to add remaining spools of planned week as per Weekly production file in this same chart?" then, after a design back-and-forth (an initial idea to overlay the Painting Plan's own "Planned Week" field - Painting's own workbook doesn't have one), landed on the real ask: "What I want to show is how old is the spool pending for PDI that was taken into the planning for production along with how old RFP'd spools are also pending for PDI. We can use Week No. as the base irrespective of how it is viewed for RFP or Planned spool."

**New third data source.** "Week Planned" lives in the Weekly Production Planning workbook's Master Planning Sheet (column BT, `src/constants.py -> WEEK`) - a workbook Painting's pipeline had never read before (it only reads the Fabrication/DPR file and its own Painting Weekly Plan workbook). Confirmed before writing any code that this is NOT the same staleness risk as Painting's own workbook: `src/pipeline.py -> run()` already reads it every cycle as part of the reliably-synced `projects` upload folder (the same one DPR itself comes through) - unlike `data/upload/painting/`, which only gets a fresh file when the person manually re-uploads one.

- `src/painting/reader.py -> read_planned_weeks()` (new): calls `ExcelReader().read_planning()["master_sheet"]` (the exact same call `src/pipeline.py` already makes for the core dashboard) and returns `{composite_key: "Week N"}` for every spool with a Week Planned value. Best-effort, matching `read_dpr_rfp_spools()`'s own contract - an empty `{}` (never raises) if the Planning workbook can't be read, so a problem here can't take down the rest of the Painting bundle.
- `src/painting/summary.py -> merge_spools()` / `_build_record()`: threaded a new `planned_weeks` dict through, stamping `week_planned` onto every merged record (`None` where the spool wasn't found in the Planning workbook - roughly 6% of RFP-done spools in the real file, confirmed).
- `src/painting/pipeline.py -> run()`: new `planned_weeks` parameter (same injection pattern as the existing `dpr_rows` one, for tests/local runs), calls `read_planned_weeks()` when not supplied, passes it into `merge_spools()`.

**New series, not a new chart.** `build_weekly_trend()` now also computes `planned_open_surface_area`: same "still pending PDI" (`not is_complete`) filter and same surface-area summing as the existing `open_surface_area`, but grouped by the spool's own Week Planned instead of its RFP week - converted to the same fiscal-week sort-key space via a new `_planned_week_sort_key()` helper (built on `utils.week_number_to_start_date()`, which already exists for exactly this "convert a planning workbook's raw week number to a real date" conversion). The two series don't share a population and don't need to - a spool's planned week and RFP week are usually different numbers (planning happens before RFP), so `all_weeks` is now the union of three key sets (completed / RFP-open / planned-open), not just one - a week with only planned-but-not-yet-RFP'd spools now shows up on the chart with a bar and no line, correctly.

**Chart** (`painting-charts.js -> renderTrend()`): now three data series plus the ideal line - "Pending PDI, by planned week" (bar, left of each week's pair) and "Pending PDI, by RFP week" (bar, right), both on the existing right-hand axis, plus the median-days line (left axis, unchanged position). Colors and styling were worked out live against a real-data preview built with the `mcp__visualize` tool (real numbers pulled from the published bundles, not mockup data) before touching any site code - several rounds of person feedback landed on: both bars semi-transparent with a solid-color outline so the line stays visible through them; planned-week bar in `PAINTING_CONFIG.plannedPendingColor` (`#8A3E82`, deep magenta - an amber/yellow first draft was rejected: "please use some other color instead of yellowish"); median line recolored from the chart's original DEE red to a neutral grey, `PAINTING_CONFIG.medianTrendColor` (`#5F5E5A`) - deliberately a NEW named color rather than reusing `overIdealColor`, which still carries its "over the ideal" meaning everywhere else on this page; gridlines removed on all three axes per direct request. `painting.html` hint text updated; `painting-config.js` and `painting-charts.js` versions bumped.

**Real numbers, not projected** - the story the chart now tells: several recent weeks (17-23) show a MUCH larger planned-week bar than RFP-week bar for the same week number (e.g. Week 17: 298 m² planned-pending vs 10.9 m² RFP-pending) - spools already committed to the production schedule for that week, not yet even RFP'd, invisible to the RFP-only view entirely. This is exactly the signal the person was after: whether the painting backlog looks better than it really is because the RFP-only view can't see work that's scheduled but hasn't reached the shop floor yet.

### 2026-09-15 - Quality dashboard: new "Open Rework & Hold by Project" chart, sourced from the Rework Data workbook directly

An ad-hoc data investigation (Projects page's Stage Ageing Summary, VOGT FP's inflated RFP dwell time, later a spool-level QC audit against the Inspection Data workbook and DPR) led into a feature request: "put a chart in Quality section for open Rework and Holds. The summary should be Project wise." Confirmed the classification rule directly with the person before building: "FQC Accept, Packing Release, RFP" = Accept, "Not Found, Rework" = Rework, "Project Hold, Query" = Hold - exactly `src/rework_pdqc_rule.py`'s existing `_STATUS_MAP`, so no new classification logic was written; the new function just reuses `normalize_rework_status()`.

**Deliberately NOT sourced from the Inspection Data workbook**, unlike every other chart added to this dashboard since 2026-09-02 - the person pointed at `Production Rework Data.xlsx` specifically, and asked for "current open" status off that workbook's own "Packing Release Date" column. `src/quality/summary.py -> build_open_rework_hold_by_project(dataframe, project_names)`: for each spool, takes only its LATEST offer event (by Prod Offer Date, the same "last row wins" stable-sort rule used everywhere else in this codebase), classifies it Accept/Hold/Rework, and counts Rework/Hold separately per project - Accept is excluded, so the chart only shows what's currently open. `src/quality/pipeline.py`: wired in as `"open_rework_hold_by_project": build_open_rework_hold_by_project(sources.rework, sources.project_names)` - `sources.rework`, not `inspection_data`, called out explicitly in a comment since it's the one exception on this page.

**Chart**: stacked bar, one bar per project (Rework + Hold stacked so the total bar height reads as "total currently open" at a glance), two-line "Project Name / (Code)" x-axis labels matching the existing Rework by Project chart's convention. New `website/js/quality-charts.js -> renderOpenReworkHoldByProject()`, new section in `website/quality.html` right after "Rework by Project", new `openReworkHoldByProject` field wired through `quality-data.js`. Versions bumped on `quality-config.js`, `quality-data.js`, `quality-charts.js`.

**Design correction, same day**: the first version used a new red/amber color pair and picked up chartTheme.js's global bar-chart default (`borderRadius: 6`, `borderSkipped: false`) unmodified - on a STACKED bar that rounds every corner of both segments, including where Rework meets Hold, which looked broken right at the seam. The person flagged it directly: "check the bar design, it looks quite bad. Use the same color combo as previous charts and keep the bar in perfect rectangular shape without rounded corners." Fixed by reusing `QUALITY_CONFIG.projectBarColor` (indigo `#4333A5`) / `otherColor` (grey `#8A8FA6`) - the exact pair the Rework by Project chart directly above it already uses - and setting `borderRadius: 0` on both datasets to override the global default (every other bar chart on this page is single-series per bar, so the rounding never caused a visible problem there). The never-shipped `QUALITY_CONFIG.holdColor` addition was removed as unused.

Verified against the real `Production Rework Data.xlsx` (W: drive) before shipping: 6 projects currently have open Rework/Hold (166 spools total - 137 Rework, 29 Hold), largest being NE Gregory (TJ/25-26/189) at 75 Rework + 17 Hold. Note this master workbook's own data only goes up to 2026-09-11 - a few days behind a fresher per-project export the person had shared earlier the same session - so the live chart's numbers will shift slightly once a newer sync picks up an updated copy; this is a data-freshness fact about the source file, not a bug in the chart.

Verified in a browser rather than by reading the diff, same as every other website-only change in this file's history (this page's Firebase auth-guard makes testing the full live page locally impractical) - built a real bundle from the actual `build_open_rework_hold_by_project()` output (not mock data) into a local auth-stripped copy of `website/`, served it, and confirmed via Chart.js instance inspection that the rendered dataset values matched the Python output exactly, no console errors.

**Housekeeping note**: this session's edits were first made by mistake directly in the primary `live-spool-status` checkout, which was found to be 3 commits behind `origin/main` (HEAD `dc35131` vs. `origin/main` `057d0f9`) - the same stale-checkout gotcha documented in earlier NEXT-SESSION.md entries. Caught before committing; the stray edits were reverted there (`git checkout --`) and redone cleanly in a fresh worktree (`../lss-worktree-quality`, branched from `origin/main`) before this entry's commit. Future sessions: always check `git log -1 --oneline` against `git log -1 --oneline origin/main` before editing the primary checkout, or just work in a fresh worktree from the start.

### 2026-09-15 (cont'd) - Open Rework & Hold by Project: added a "Download Open Rework & Hold" button

Asked directly after the chart shipped: "Can you add a button to download current hold and rework excel file on this chart? I want to show type of rework and qc observation." - `Rework Type` and `QC Observation` are raw Rework Data workbook columns already read by `sources.rework`, just never surfaced on this chart (the aggregated per-project counts have no room for row-level free text).

New `src/quality/summary.py -> build_open_rework_hold_export(dataframe, project_names)`: same population, same latest-offer-event rule as `build_open_rework_hold_by_project()` right above it (deliberately not shared/refactored into one function - each `build_*` function in this file is already self-contained, and the "latest event per spool" snippet is short enough that duplicating it keeps both functions independently readable, same reasoning `rework_pdqc_rule.py`'s own separate copy of this pattern already follows) - but returns one row per currently-open spool instead of counts, via `utils.dataframe_to_json_records()` (already used elsewhere in this pipeline, not reimplemented): Project Name, Project Code, Drawing No, Spool No, Prod Offer Date, Status, Rework Type, QC Observation, Prod Engineer. Wired into `pipeline.py`'s bundle as `open_rework_hold_export`.

**Button**: `website/quality.html`'s "Open Rework & Hold by Project" section head restructured (title+hint wrapped in one div, button as the second flex child, `position:static` override) so the button sits top-right of the section without `.btn-export`'s default absolute positioning fighting the hint text - same technique the "Download Production Rework Data" button already uses one section up. New `website/js/quality-charts.js -> wireOpenReworkHoldExportButton()` / `exportOpenReworkHoldWorkbook()`, copying `wireReworkExportButton()`/`exportReworkWorkbook()`'s exact client-side, no-second-Python-pass pattern (SheetJS, one sheet, no monthly summary needed for this one). New `openReworkHoldExport` field wired through `quality-data.js`. Versions bumped (`quality-data.js`, `quality-charts.js` -> `20260915b`).

Verified against the real `Production Rework Data.xlsx`: 166 rows (matching the chart's own 137+29 total), correct 9-column header, spot-checked the first row's Rework Type ("Dimension") and QC Observation text against the source workbook. Verified in a browser, same auth-stripped local-copy method as every other website-only change in this file - clicked the button live (no console errors) and independently rebuilt the same workbook via the browser's own JS console to confirm the exact header row and row count SheetJS would write.

### 2026-09-15 (cont'd) - Removed the "Showing the latest published data" toast

The person noticed a black toast box flashing for 1-2 seconds on every page refresh and asked what it was - `website/js/quality-app.js -> loadInitialData()` fires it every time the page successfully fetches the published bundle, which is the routine, expected path on every load. Asked what to recommend: pointed out the header's "DATA AS OF" timestamp already confirms freshness permanently, and a toast is most useful for the OTHER paths in that same function - a restored manual upload, a local-only preview, an error - where what's on screen might not be what's expected. The person agreed: "yes, go ahead."

Removed the `this.showToast("Showing the latest published data")` call on the successful-fetch branch only; the upload/preview/error toasts elsewhere in the same file are untouched. Verified in a browser (auth-stripped local copy, same as every other website-only change here): toast element stays `is-visible: false` and empty after a successful load, header timestamp still populates correctly, no console errors. Version bumped: `quality-app.js` -> `20260915`.

Verified against the real published bundles (painting bundle + the Projects/master bundle, standing in locally for a real `read_planned_weeks()` read against actual Planning-workbook data, since the raw workbook itself isn't available on this laptop - same constraint documented in the housekeeping note below) - the actual `build_weekly_trend()` function, not a reimplementation, produced numbers matching the live-data preview shown to the person before building. Local preview rendered with no console errors, `body.is-ready` set, and dataset order/colors/axis confirmed via both Chart.js instance inspection and a screenshot.

### 2026-09-21 - Fabrication Line: new "Spools Planned in Next Week" card, plus a long-standing card-width bug fixed

Asked directly: the Fit-Up card on the main dashboard's Fabrication Line counts every spool with a released production order that hasn't started Fit-Up yet - "planned and not started" and "not due to start until next week" both land in the same bucket today, and the person wanted them told apart: "the spools that are planned for next week, should not be shown here [Fit-Up card]... add a card between Production Order Not Released and Fit-up... called Spools planned in next week." Confirmed the exact scope before building: today (2026-09-21) is fiscal Week 26, so "next week" = Week 27, matching the person's own worked example exactly.

`src/summary.py -> generate_dashboard_summary()`: new module constant `PLANNED_NEXT_WEEK_LABEL = "Spools Planned in Next Week"`. After the existing `stage_counts` value_counts, a spool counts toward this new bucket if its `Current Stage` is the first configured stage (`Fit-Up`) AND its `Week` field (already on every spool via the Weekly Production Planning workbook - no new data plumbing needed) equals next week's fiscal label, computed as `fiscal_week_info(today() + timedelta(days=7))["week_label"]` - reusing the exact same fiscal-week machinery every other "Week N" comparison in this codebase already uses, so it rolls over correctly at a fiscal year boundary with no special-casing. That count is subtracted from `Fit-Up`'s own count and added as a new key. Deliberately does NOT touch the `Current Stage` column value stored on each spool - only the `current_stage_distribution` dict this ONE function returns is reshaped, so every other KPI/chart that reads `Current Stage` (average age, hold-by-project-stage, group/project summaries, the Production dashboard's own independent classification) is completely unaffected, per the person's explicit "no other chart/data needs to be changed."

Real numbers today: of 1,004 spools at Fit-Up, 674 are tagged Week 27 - Fit-Up drops to 330 (the genuinely-overdue count), Spools Planned in Next Week shows 674.

**Card position and color are frontend-only**: `website/js/config.js -> SPOOL_STATUS_CONFIG.stageOrder` (the array that actually decides render order in `fabline.js` - the backend dict's key order doesn't matter) got the new label inserted between "Production Order Not Released" and "Fit-Up"; `stageColor` got a new sky-blue `#2E86C9`, distinct from its two neighbors. `website/js/fabline.js -> EXCLUDED_FROM_BOTTLENECK` also got the new label added, per the person's confirmation ("Yes, correct reasoning") that a not-yet-due card shouldn't ever earn the "Bottleneck" badge, same treatment as "Production Order Not Released" already gets.

**Also fixed, same request**: the person separately flagged that long card labels ("Production Order Not Released", "Ready for Painting") were forcing their cards wider than every other card in the row. Root cause: `.fabline__stage-name` had `white-space: nowrap`, which makes a flex item's content-based sizing equal to the FULL unwrapped line width (nowrap collapses min-content and max-content to the same value) - and since `.fabline__stage` has `flex-shrink: 0`, that box could never be sized below that width no matter what `flex-grow` said. The `overflow: hidden; text-overflow: ellipsis` sitting right next to `nowrap` never actually fired, because the box was never constrained smaller in the first place. Fixed in `website/css/styles.css` by letting the label wrap (`white-space: normal; overflow-wrap: break-word`) instead - wrapping gives the browser a much smaller min-content width (the longest single word) to size the box from, so every card's width goes back to being governed by `flex-grow`/count alone, exactly like the short-labeled cards always were.

Verified in a browser (auth-stripped local copy of `website/`, same method as every other website-only change in this file): rebuilt `dashboard_summary` from the real published bundle's `master_spools` through the actual updated `generate_dashboard_summary()` (not a reimplementation) - confirmed via `SpoolData.store.dashboardSummary.current_stage_distribution` in the browser's own console that the rendered numbers matched exactly (Fit-Up 330, Spools Planned in Next Week 674, all others unchanged). Screenshot confirmed: new card renders in the right position with its own color, "Production Order Not Released" now wraps onto two lines instead of stretching its card, "Bottleneck" badge correctly still lands on "Under Painting" (746, the real busiest stage) and not on the new card. No console errors.

### 2026-09-21 (cont'd) - Fabrication Line row was still overflowing on real desktop screens - redesigned as a uniform 2-row grid

The wrap fix above was verified at a narrow (710px) test viewport, where a completely different, pre-existing rule (`.fabline { flex-wrap: wrap }`, only active below 720px) was already wrapping cards onto multiple rows - masking the real desktop behavior entirely. The person reported the live page still showed cards cut off ("it is still not wrapped, and overflowing") with a screenshot from an actual desktop-width browser. Reproduced properly this time at 1440px: `fabline.scrollWidth` (1430px) exceeded `fabline.clientWidth` (1320px) by 110px, with `overflow-x: visible` and no scrollbar - the excess silently spilled past the section and got clipped by an ancestor. Root cause wasn't the wrap fix itself (nowrap would have been worse) - adding an 11th stage card tipped an already-tight width budget over the edge, since several labels still need 150-225px even wrapped onto 2 lines, and `flex-shrink: 0` meant nothing could compress to fit.

Presented 3 options (horizontal scroll / let the row wrap onto more rows / shrink everything to force-fit) and asked which the person wanted before touching anything further, rather than guessing again. Given a fourth answer instead: "Make 2 rows and make the cards of equal shape and size so that it looks balanced and good" - dropping the "width shows count" visual metaphor entirely in favor of a uniform, balanced grid.

**`website/js/fabline.js`**: removed the `flex-grow`-proportional-to-count sizing entirely (`const proportion = count / maxCount; block.style.flexGrow = ...`) - card size no longer reflects spool count at all; the Bottleneck badge + the count number are what carry that signal now. Added one line setting the container's `grid-template-columns` inline to `repeat(Math.ceil(stageCount / 2), 1fr)`, so the row is always exactly 2 rows no matter how many stages are configured, not a hardcoded "6".

**`website/css/styles.css`**: `.fabline` changed from `display: flex` to `display: grid` (column count set by the JS above); `.fabline__stage` given a fixed `height: 108px` (same value the old flex row already used) so every card is now identical in both width (equal grid columns) and height - "shape and size" both uniform, per the person's exact wording. The mobile breakpoint's old flex-basis rule was replaced with a 3-column override on `.fabline` using `!important` (the only way to beat the JS-set inline `grid-template-columns` at that breakpoint) - phone-width screens get more, narrower rows instead of forcing 6 columns.

**`website/dashboard.html`**: the section's own descriptive copy ("Width shows how many spools are sitting there. The widest block is where work is piling up.") was now describing a mechanic that no longer exists - updated to "Each block is a stage and shows how many spools are sitting there right now. The busiest stage is flagged Bottleneck."

Verified at both a real desktop width (1440px) and mobile (375px), same auth-stripped local-copy method: at 1440px, `fabline.scrollWidth === fabline.clientWidth` (1320 === 1320, zero overflow) and every one of the 11 cards measured exactly 213×108px - perfectly uniform. At 375px, the `!important` mobile override was confirmed via `getComputedStyle` to actually win (3 columns, 92px each), also zero overflow. Bottleneck badge on "Under Painting" no longer overlaps the stage name (cards are wide enough now regardless of label length). No console errors at either width.

### 2026-09-21 (cont'd) - Fabrication Line card wording changed to read as active/in-progress states

The person asked to reword several of that same row's card names: "use words such as Under Fit-Up, Under Welding, Under PDQC, Pending Ready for Painting, Under Packing, Under Dispatch, for Completed please use Shipment complete." Confirmed the exact mapping before touching code, since "Completed" -> "Shipment complete" changes the word entirely rather than just prefixing it.

This is display wording only - the underlying `Current Stage` value every spool carries (`"Fit-Up"`, `"Welding"`, `"PDQC"`, `"Ready for Painting"`, `"Packing"`, `"Dispatch"`, `"Completed"`) is untouched everywhere else it's used: the All Spools table, its filters, CSV exports, the Production dashboard's own stage tracking, and `current_stage_distribution`'s own keys. Renaming those strings directly would have silently broken every one of those.

**`website/js/config.js`**: new `SPOOL_STATUS_CONFIG.stageDisplayLabel` lookup, consulted only by `fabline.js`'s rendering: `Fit-Up -> "Under Fit-Up"`, `Welding -> "Under Welding"`, `PDQC -> "Under PDQC"`, `Ready for Painting -> "Pending Ready for Painting"`, `Packing -> "Under Packing"`, `Dispatch -> "Under Dispatch"`, `Completed -> "Shipment Complete"`. A stage not listed (e.g. "Production Order Not Released", "Spools Planned in Next Week", "Partial Fit-Up/Welding", "Under Painting" - which is already the raw stage name, so it happens to need no entry) just displays its own raw name, unchanged.

**`website/js/fabline.js`**: the card name, its hover tooltip, and the "Busiest stage right now" note's bolded stage name all now resolve through `stageDisplayLabel[stage] || stage` instead of the raw `stage` string directly. Every lookup that has to match the real data (`distribution[stage]`, `stageColor[stage]`, the bottleneck exclusion list) still uses the untouched raw `stage` value - only the three places text actually reaches the screen were changed.

Verified in a browser (auth-stripped local copy, same method as prior entries): rebuilt `dashboard_summary` from the real published bundle through the actual `generate_dashboard_summary()`. Confirmed via `document.querySelectorAll('.fabline__stage')` that all 7 renamed cards show the new wording in both their visible label and `title` tooltip (e.g. "Under Fit-Up: 277 spools"), and that the four unmentioned stages ("Production Order Not Released", "Spools Planned in Next Week", "Partial Fit-Up/Welding", "Under Painting") rendered unchanged. That day's real bottleneck happened to land on "Under Painting" (unmapped), confirming the note's fallback-to-raw-name path works correctly for a stage with no display label. No console errors.

### 2026-09-22 - English/Thai language toggle, site-wide

The person floated this once before (see the 2026-09-21 handoff, "parked, not implemented") and brought it back up. Scoped it with two questions first: which pages (answer: all of them) and how deep - UI chrome only (nav, headers, labels, buttons, tooltips, static copy) vs. also translating chart/table-generated content. Chose UI chrome only: dynamically-rendered chart legends/axis labels and `<table>` column headers stay English, since those are generated per-chart across a dozen-plus JS files and would have made this a much larger, riskier change for comparatively little gain (a Thai-speaking floor supervisor mainly needs the page's labels and headings, not axis tick text).

**New `website/js/i18n.js`**: mirrors the existing `theme.js` pattern (immediately-invoked module, a visible pill-button widget mounted into `<div class="lang-toggle" id="lang-toggle"></div>`, `window.I18N` exposed for other scripts) but deliberately differs on one point - language is stored under a single SITE-WIDE `localStorage` key (`lang`), not per-page like theme, because a person's language is a property of them, not of which dashboard tab happens to be open. A ~240-entry `DICT` of `{ key: { en, th } }` pairs, an `apply(lang)` pass that walks `[data-i18n]` (textContent), `[data-i18n-title]`, `[data-i18n-placeholder]`, `[data-i18n-aria-label]` and re-runs instantly on every toggle click (no page reload needed, unlike the theme toggle - nothing here touches a Chart.js canvas).

**Every page** (`index.html`, `dashboard.html`, `production.html`, `quality.html`, `packing-dispatch.html`, `painting.html`, `login.html`) got the toggle widget wired in next to the theme toggle (top-right, fixed position on `login.html`, which has no header), plus `js/i18n.js` loaded after `js/user-menu.js` - that ordering matters: `user-menu.js`'s own `DOMContentLoaded` handler (which builds its portaled dropdown and snapshots its "Logout" label text) has to run before `i18n.js`'s translation pass, or the dropdown's dynamically-built markup wouldn't have `data-i18n` attributes yet to translate. `website/js/user-menu.js` itself got `data-i18n`/`data-i18n-title` attributes added to its templated HTML, and its "Signed in as"/"Logout"/"Click to confirm"/"Couldn't sign out" strings now resolve through `window.I18N.t()` at the moment they're shown rather than being hardcoded - the "Logout" default-label reset in particular had to become a function call instead of a value snapshotted once at build time, otherwise it would freeze in whichever language was active before `i18n.js`'s own pass ran.

**Coverage differs by page**, proportional to how much was already open in this session: `index.html` (hub) and `dashboard.html` (Projects) got full topbar + KPI strip + every chart-card heading + tabs + filter-bar labels + footer. `packing-dispatch.html` and `painting.html` got the same depth (topbar, KPI strip, every chart-card heading, tabs, filter labels, footer). `production.html` and `quality.html` got topbar + KPI strip + footer + (production only) the global chart filter labels, but NOT their own chart-card headings (e.g. "Spool Distribution", "Top 10 Rework Types" are still English) - a scope cut made to keep the session tractable, not a technical limitation. `login.html` got its tagline, field labels, submit button, and all 5 Firebase auth error messages translated. Across every page, `<table>` column headers, dynamically-populated dropdown option lists (project/week/material/etc.), and Chart.js-rendered text (axis labels, legends, datalabels) are unchanged - by design, per the UI-chrome-only scope.

**Bug caught during verification, fixed**: three files rebuild a project-filter `<select>`'s default option at runtime from a hardcoded English string, discarding whatever `data-i18n` markup was there originally the moment real data loads - `website/js/stageAgeing.js` (`dashboard.html`'s Stage Ageing "Project" filter), `website/js/packing-tables.js` (`packing-dispatch.html`'s table project filter), `website/js/painting-tables.js` (`painting.html`'s table project filter). All three now build that option's text through `window.I18N.t("common.allProjects")` (with an English fallback if `I18N` somehow isn't loaded) and keep the `data-i18n` attribute on the freshly-built element, so a language toggle *after* the dropdown repopulates still re-translates it correctly. Found by literally reading the live-data preview's page text after toggling to Thai and noticing "All Projects" hadn't translated on two of three project-filter dropdowns - `pending-project-filter` on `painting.html` was unaffected because that one only ever `appendChild`s new options and never touches the existing default option's markup.

Verified in a browser (auth-stripped local copy of `website/`, same method as prior entries, against the repo's real published data bundle - 13,139 real spool rows on the Projects dashboard, not synthetic data): loaded all 7 pages, toggled EN↔TH on each, confirmed the translated text swaps instantly with no reload and no console errors, confirmed the language choice persists across page navigation (site-wide `localStorage` key, as designed), and specifically re-checked all three "All Projects" dropdowns after the bug fix. Every `data-i18n*` reference across every page cross-checked programmatically against `DICT`'s keys - zero missing. Pushed to `origin/main` as commit `6cc5f34` after the person confirmed.

### 2026-09-22 (cont'd) - Language toggle: brought Production and Quality up to full depth

The person asked "are all pages covered?" after the previous entry's push, which surfaced the known gap directly - confirmed `production.html`/`quality.html` only had topbar/KPI/footer, not their own chart-card headings. Asked to bring those two up to the same depth as the other four pages; they said yes.

Added ~55 more `DICT` entries to `website/js/i18n.js` (production.* and quality.*) and `data-i18n` attributes to every remaining static heading/button on both pages: every `charts-grid__head`/`chart-card__head` `<h3>` (Spool Distribution, Ideal vs. Actual Time, Delayed vs. In Time by Project, the full Backlog by Operation sub-tree, Currently on Hold, Currently in Rework, the entire (currently `hidden`) Material Handover section including its own 4-card KPI strip, Stage-wise Ageing by Category's 6 joint-count sub-cards, on `production.html`; Overview, Top 10 Rework Types, First Offer Outcome, Rework by Project, Open Rework & Hold by Project, Trend & Repeat Cycles, Rework Rate Over Time, the (currently `hidden`) Welder Performance section's 5 sub-charts, on `quality.html`), every `Export to Excel`/`Download ...` button (reusing the existing `common.exportToExcel` key where the text is identical), the "Group by"/Day/Week/Month trend-granularity control on `quality.html`, and `production.html`'s Spool List table chrome - the heading, "Clear All Filters", and the "← Prev"/"Next →" pagination buttons (the arrow glyphs stay outside the translated `<span>` so they don't get swapped with the text). Table `<th>` headers and everything Chart.js draws stay English, same scope boundary as every other page.

Checked before assuming it was safe to translate: grepped every touched button/heading's `id` against `production-charts.js`, `production-table.js` for any `.textContent =`/`.innerHTML =` reassignment that would silently clobber a translation the way the previous entry's bug did - none of the export/download/pagination buttons on either page get their displayed text rewritten at runtime (only `.disabled`/`.onclick`, or a separate `dataset.backlogLabel` used purely for the exported Excel sheet name, untouched by the `data-i18n` attribute added alongside it).

Verified the same way as the initial rollout: auth-stripped local preview against the real published bundle (10,121 rows on Production, real Quality rework data), EN↔TH toggle on both pages, no console errors, and the same programmatic zero-duplicate/zero-missing-key check re-run against the now-297-entry dictionary. Pushed to `origin/main`.

### 2026-09-23 - Design polish pass: Projects (clean), Painting and Production (two real mobile/overflow bugs fixed)

The person asked for an Impeccable-skill "polish" pass. Scoped it first rather than running loose across all 7 pages on a live production tool - agreed to start with the Projects dashboard. That page turned up nothing: checked desktop (1440px) and mobile (390px) layout, dark mode, text contrast on every secondary/hint label (measured programmatically, 5.3-6.8:1, all clear of WCAG AA), focus-ring visibility, and console/network (136 requests, zero errors) - all clean, consistent with how much prior work this exact page has already been through per this file's own history (contrast fixes, overflow fixes, badge positioning, entrance animation, dark-mode tokens). Reported that honestly rather than inventing changes to justify the pass, and asked what to check next; the person named Painting and Production.

Same method on both: real published data, desktop + mobile + dark mode, contrast measured not eyeballed, console/network checked. Two initial "finds" turned out to be false alarms worth recording so a future pass doesn't re-chase them - a seemingly cramped 10-project chart legend on the earlier Projects check (verified via Chart.js instance inspection: canvas dimensions were proportionate, just hard to judge from a downscaled screenshot) and an apparently 862px-tall, oddly oversized "Internal vs External Blasting" chart card on Painting (verified via the chart's own config: `indexAxis: 'y'`, 20 weekly bar-pairs, `maintainAspectRatio: false` - the height is deliberately data-driven, already explained in a `painting.css` comment from 2026-09-04, not a bug).

Two real ones, both fixed:

**`website/css/painting.css`** - the 8-tab table-toolbar tablist (`All RFP-Done Spools` through `In Plan, Not RFP in DPR`) wraps via `flex-wrap: wrap` once it doesn't fit one row, which is true at almost every real viewport width - the tabs' natural content width (~1340px) only fits below the site's own 1440px `--max-width` container, and 8 items of uneven width wrapping via flexbox pack however many fit per line, producing a ragged 2-then-1-then-2-then-1 pattern rather than a scannable list. Confirmed this was specific to this page (not a shared-CSS problem): `packing-dispatch.html`'s 4 tabs fit one row cleanly at 390px and were left untouched. Fix: an unconditional 2-column CSS grid for `.tables-toolbar .tabs` on this page only, so every tab is the same width and rows always align - deliberately not a pixel breakpoint chasing the single-row case, since (a) the margin between "fits" and "wraps" is only ~100px wide at exactly the container's own max-width, and (b) these same 8 tabs now have Thai translations (this file's own EN/TH work, one entry up) that shift label length, which would keep invalidating any fixed breakpoint. Verified at 390px, 1000px, 1400px, and dark mode - clean aligned grid at all of them, tab click/pane-switch interaction still works, `impeccable detect --json` clean.

**`website/css/production.css`** - a real, user-facing bug, not just cosmetic: the global chart filter bar's "Project" group (label + the multi-select, which has its own `min-width: 160px`, + the "All Projects" reset button) is ~385px wide, wider than a ~360px mobile content area. The parent bar's `flex-wrap: wrap` only wraps whole groups onto new lines, not the items inside one group, so the oversized group's own overflow got silently clipped by the filter bar's rounded corners - the "All Projects" button was fully invisible on a phone, not just cramped. Fix: `flex-wrap: wrap` added to `.production-global-filters__group` itself, so the button drops to its own line instead of disappearing. Verified at 390px (button now fully visible and clickable, `scrollWidth` now equals `clientWidth`) and 1440px (still one row, no regression - `impeccable detect --json` clean on both changed files).

Not yet pushed to `origin/main`; waiting on the person's go-ahead.
...
Verified in a local auth-stripped preview: legend reads six items, dataset totals match the raw stage sums (773+208+78+185 = 1244 Production; 101+4714 = 4815 Packed), Weekly Progress legend still shows all eight raw stages, no new console errors. No Thai strings were added (the chart legend takes its labels straight from the data, not from `i18n.js`) - if Thai labels for these three names are wanted, that is a separate small change.

### 2026-09-25 - Fabrication Line: "Planned" card now counts every future planned week

Per the person: the card previously counted only spools at Fit-Up whose Week equalled today+7 (Week 27 when today was Week 26 = 674), so the 464 Week 28 spools stayed inside plain Fit-Up. They plan 3-4+ weeks ahead, so it now counts every non-hold Fit-Up spool whose planned Week number is greater than the current fiscal week (`src/summary.py -> generate_dashboard_summary()`; week number parsed from the "Week N" text). Against the 2026-09-25 published data that gives 1,138 (674 + 464), leaving Fit-Up at 60. The `current_stage_distribution` key is unchanged ("Spools Planned in Next Week"); the card now *displays* as "Spools Planned in Future Weeks" via `stageDisplayLabel` in `website/js/config.js`. Known limit: Week labels carry no year, so near the fiscal-year wrap (e.g. today Week 51, plan for Week 3) those spools would not count as future. Takes effect on the next pipeline run/sync; verified the mask logic against the published master_spools, not a full pipeline run.

### 2026-09-25 (cont'd) - Drive sync now triggers the website deploy

Found while checking a sync: `drive-sync.yml` commits fresh `website/data/*.json`, but it pushes with the default `GITHUB_TOKEN`, and GitHub does not fire other workflows for such pushes - so `deploy.yml`'s `on: push` never ran and the live site kept serving the previous data until someone dispatched "Deploy Website" by hand. Fix: `drive-sync.yml` gained `actions: write` permission and, right after its `git push`, runs `gh workflow run deploy.yml --ref main` (only when a commit was actually made). Not yet proven on a scheduled run - confirm after the next hourly sync that a "Deploy Website" run (event `workflow_dispatch`) appears right after it.
